...
use App\Models\Ecdata;
use Illuminate\Support\Facades\DB;
use App\Models\Score;
use App\Models\EcAllLog;
use App\Models\EcOwnLog;
...
class ammendmentNidOcrController extends Controller
{
    ...
    public function ammendmentOcr(Request $request){
        ...
        $nid_number  = str_replace(" ","",$request->input('nid_number'));
        $customer    = Customer::find($customer_id);

        $customer->bn_name         = $request->input('bangla_name');
        $customer->en_name         = $request->input('english_name');
        $customer->father_name     = $request->input('father_name');
        $customer->mother_name     = $request->input('mother_name');
        $customer->date_of_birth   = $request->input('date_of_birth');
        $customer->nid_number      = $nid_number;
        ...
        $customer->nid_unique_data = $request->input('nid_unique_data');
        $customer->user_id         = Auth::user()->id;
        $customer->updated_at      = date('Y-m-d H:i:s');
        $customer->save();

        // get election commmissiont date
        $ec_id              = $this->getElectionCommissionData( $customer_id, $nid_number );
        $compare_percentage = $this->showMatchingScore($customer_id, $ec_id);
        return $compare_percentage;

    }


    // Fetch Election CommissionData

    public function getElectionCommissionData($customer_id, $nid_number){
        $ec_controller = new EcController();

        $seachCustomerFromOurECTable = DB::table('ecdatas')->select('id', 'nid_number')->where('nid_number',$nid_number)->first();
        if(isset($seachCustomerFromOurECTable->nid_number) != null){
            $ec_controller->ecAllHitLog($nid_number . " fetch out database", Auth::user()->company_id, Auth::user()->id ); // response send
            return $seachCustomerFromOurECTable->id;            
        }else{
            $ec_controller->ecActualHitResponseLog($nid_number, Auth::user()->company_id, Auth::user()->id);
            $ec_controller->ecAllHitLog($nid_number . " fetch from ec", Auth::user()->company_id, Auth::user()->id); // response send
            $ecResponseByNidNumber = $ec_controller->nidECReponseData($nid_number);
            $ec_id                 = $ec_controller->saveEcResponse($ecResponseByNidNumber, $customer_id, $nid_number);
            return $ec_id;
        }   
    }
